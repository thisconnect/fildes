...
test.only('no win error', t => {
  const fs = require('fs');
  fs.writeFileSync('./test2.txt', 'filecontent');
  console.log('should error')
  fs.mkdirSync('./test2.txt/should/error/here', {
    recursive: true
  });
  t.end()
});
...
test('windowserror', t => {
  const fs = require('fs');
  fs.writeFileSync('./test2.txt', 'filecontent');
  // fs.mkdirSync('./test.txt/test');
  const { promisify } = require('util');
  makeDir('./test2.txt/test')
    .then(() => {
