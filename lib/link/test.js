...
test.only('link dest error', t => {
  const filepath = resolve(dir, './link-dest-error.txt');
  const dest = resolve(filepath, 'sub/not/work.txt');

  write(filepath, 'link test\n')
    .then(() => link(filepath, dest))
    .then(() => {
      t.fail('should not link');
      t.end();
    })
