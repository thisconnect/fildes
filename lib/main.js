...
exports.open = function(path, o) {
  ...
  return open(path, flags, mode).catch(function(err) {
    if (['w', 'w+', 'a', 'a+'].indexOf(flags) == -1) {
      throw err;
    }
    return extra.mkdir(dirname(path)).then(function() {
...
var writeBuffer = function(fd, buffer, offset, length, position) {
  return new Promise(function(resolve, reject) {
    ...
    fs.write(fd, buffer, offset, length, position, function(
      err,
      written,
      buffer
    ) {
  ...
  return exports
    ...
    .then(function(fd) {
      return write(fd, data, o)
        ...
        .catch(function(err) {
          return finish(fd, path).then(function() {
            throw err;
          });
        });
    });
...
exports.read = function(path, buffer, o) {
  ...
  return open(path, o.flag || o.flags || 'r').then(function(fd) {
...
exports.chown = exports.fchown = function(path, o) {
  ...
  return open(path, o.flag || o.flags || 'r+').then(function(fd) {
    var mode = o.mode;
    var uid = o.uid || (!!process.getuid && process.getuid());
    var gid = o.gid || (!!process.getgid && process.getgid());
...
exports.access = function(path, o) {
  var mode = !o ? fs.F_OK : o.mode || o;
...
exports.link = function(src, dest) {
  return access(src).then(function() {
    return link(src, dest).catch(function(err) {
      return extra.mkdir(dirname(dest)).then(function() {
...
var symlink = function(dest, path) {
  return new Promise(function(resolve, reject) {
    debug('symlink');
...
exports.symlink = function(dest, path) {
  return symlink(dest, path).catch(function(err) {
    return extra.mkdir(dirname(path)).then(function() {
