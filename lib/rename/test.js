...
test('rename error', t => {
  rename('./does/not/exits.txt', './there/is/no/such/dir/file.txt')
    ...
    .catch(error => {
      ...
      console.log(normalize(error.path));
      console.log(resolve(error.path));
      console.log(normalize('./does/not/exits.txt'));
      console.log(resolve('./does/not/exits.txt'));
      t.equal(
        normalize(error.path),
        normalize('./does/not/exits.txt'),
        'correct error.path'
      );
      t.true(
